...
std::unique_ptr<Solveur> fabriquer_solveur(int choix, const Materiau& mat) {
    double t_max = 16.0; // [cite: 72]
    double L = 1.0;
    int nb_points = 1001; // [cite: 75]
...
void afficher_rapport(const std::vector<EtapeLog>& historique, const std::string& nom_mat) {
    ...
    for (size_t i = 0; i < historique.size(); i += pas) {
        printf("|  %5.1f s    |  %8.2f   |  %8.2f   |\n", 
               historique[i].temps, historique[i].t_max, historique[i].t_moy);
    }
    if (!historique.empty()) {
        const auto& last = historique.back();
        printf("|  %5.1f s    |  %8.2f   |  %8.2f   |\n", last.temps, last.t_max, last.t_moy);
    }
    std::cout << "======================================================\n";
...
void mettre_a_jour_affichage(Sdl& sdl, Solveur* solv, int type_simu, double max_temp) {
    ...
    if (type_simu == 1) sdl.dessiner_barre(solv->get_donnees(), t_min, t_max);
    else {
        auto* s2d = dynamic_cast<Solveur2D*>(solv);
        if (s2d) sdl.dessiner_surface_2d(solv->get_donnees(), s2d->get_N(), t_min, t_max);
    }
    sdl.afficher();
}
