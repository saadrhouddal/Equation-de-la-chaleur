...
std::unique_ptr<Solveur> creer_solveur(int type_simu, const Materiau& mat) {
    ...
    if (type_simu == 1) return std::unique_ptr<Solveur>(new Solveur1D(mat, 100, L, t_max));
    else return std::unique_ptr<Solveur>(new Solveur2D(mat, 60, L, t_max));
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== PROJET CHALEUR (Rouge Stats Live) ===" << std::endl;
    ...
    while (continuer) {
        ...
        // MODIFICATION DE L'ECHELLE ICI
        // Min = 13°C (286.15)
        // Max = 15°C (288.15) -> Pour bien voir les petits changements en rouge
        ...
        if (choix_simu == 1) {
            graphisme.dessiner_barre(donnees, t_visu_min, t_visu_max);
             ...
             if (s2d) graphisme.dessiner_surface_2d(donnees, s2d->get_N(), t_visu_min, t_visu_max);
        }
